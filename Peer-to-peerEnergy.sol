...
contract PeerEnergy is DoubleSidedAuction {
    ...
    /*This function balances the consumption from each user*/
    function _balanceUserConsumption() internal {
        bool logic;  uint AmtTopay;uint enegydiff;
        uint itT=_getIterationIndex();
        if(users[userAddresstoID[msg.sender]].consumedEnergy>0 || users[userAddresstoID[msg.sender]].BQty[itT]>0){
            uint consEnWh= (users[userAddresstoID[msg.sender]].consumedEnergy)/3600+1;
            if (users[userAddresstoID[msg.sender]].BQty[itT]==0){
                AmtTopay = consEnWh*Token_rate
                          *grids[gridIDToNo[users[userAddresstoID[msg.sender]].gridNo]].utilitySellPrice;
                logic=transfer(Utility, AmtTopay); 
                _setRelayState(logic,users[userAddresstoID[msg.sender]].consumedEnergy);
                _payUserDebt(msg.sender);
            }   
            else {
                if (users[userAddresstoID[msg.sender]].BQty[itT]==consEnWh){
                users[userAddresstoID[msg.sender]].BQty[itT]=0;
                users[userAddresstoID[msg.sender]].consumedEnergy=0;
                consEnWh=0;
                }
                else if(users[userAddresstoID[msg.sender]].BQty[itT]>consEnWh){
                    enegydiff= users[userAddresstoID[msg.sender]].BQty[itT]-consEnWh;
                    AmtTopay=enegydiff*Token_rate*grids[gridIDToNo[users[userAddresstoID[msg.sender]].gridNo]].utilityBuyPrice;
                    users[userAddresstoID[msg.sender]].UtilityDebt +=AmtTopay;
                    users[userAddresstoID[msg.sender]].BQty[itT]=0;
                    users[userAddresstoID[msg.sender]].consumedEnergy=0;
                    consEnWh=0;
                }
                else{
                    enegydiff= consEnWh-users[userAddresstoID[msg.sender]].BQty[itT];
                    AmtTopay=enegydiff*Token_rate*grids[gridIDToNo[users[userAddresstoID[msg.sender]].gridNo]].utilitySellPrice;
                    logic=transfer(Utility, AmtTopay); 
                    _setRelayState(logic,users[userAddresstoID[msg.sender]].consumedEnergy);
                    _payUserDebt(msg.sender); 
                    consEnWh=0;
    ...
    /*This function balances the production from each user*/
    function _balanceUserProduction() internal {
    uint k; uint i; uint enegydiff;
       uint Qtytoproduce; uint Energyamt; 
         Qtytoproduce=0;k=0;
         Energyamt=0; 
         uint itT=_getIterationIndex();
        if(users[userAddresstoID[msg.sender]].producedEnergy>0 || users[userAddresstoID[msg.sender]].sellcount[itT]>0){
            uint ProdEnWh =(users[userAddresstoID[msg.sender]].producedEnergy)/3600;
            if (users[userAddresstoID[msg.sender]].sellcount[itT]>= users[userAddresstoID[msg.sender]].SQty[itT].length){
                 users[userAddresstoID[msg.sender]].sellcount[itT]= users[userAddresstoID[msg.sender]].SQty[itT].length-2;       
            }
            for (i = k; i<users[userAddresstoID[msg.sender]].sellcount[itT]; i++){
                Qtytoproduce =users[userAddresstoID[msg.sender]].SQty[itT][i];
                if (ProdEnWh>=Qtytoproduce){
                    Energyamt = (users[userAddresstoID[msg.sender]].SQty[itT][i])*
                    users[userAddresstoID[msg.sender]].SPrice[itT][i]*Token_rate;
                    ProdEnWh -= Qtytoproduce;
                    transferFrom(ExchangeAddr, msg.sender, Energyamt);
                }
                else{
                    Energyamt = ProdEnWh*
                    users[userAddresstoID[msg.sender]].SPrice[itT][i]*Token_rate;
                    transferFrom(ExchangeAddr, msg.sender, Energyamt);
                    enegydiff=Qtytoproduce-ProdEnWh;
                    enegydiff=enegydiff*3600;
                    users[userAddresstoID[msg.sender]].userDebt +=enegydiff;
                    users[userAddresstoID[msg.sender]].producedEnergy=0;
                    ProdEnWh=0;
                }
            }
            users[userAddresstoID[msg.sender]].sellcount[itT]=0;
            if(ProdEnWh>0){
                Energyamt = (ProdEnWh)*
                Token_rate*grids[gridIDToNo[users[userAddresstoID[msg.sender]].gridNo]].utilityBuyPrice;
                users[userAddresstoID[msg.sender]].producedEnergy = 0;
                ProdEnWh=0;
                users[userAddresstoID[msg.sender]].UtilityDebt +=Energyamt;
            }
        }
        else{
            users[userAddresstoID[msg.sender]].sellcount[itT]=0;
    ...
    /*used to tranfer and approve tokens after each match is made.*/
    function _assignMatchedPrice( uint _Avp, uint _Avq, uint _GridNo, uint  _ith, uint ir, uint ij) internal returns(uint){
        uint Amt =  _Avp* _Avq*Token_rate; 
        transferFrom(grids[gridIDToNo[_GridNo]].EBaddr[_ith][ij], ExchangeAddr, Amt);
        approve(grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir], Amt);
        if (users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].sellcount[_ith]>=
             users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].SQty[_ith].length){
            users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].sellcount[_ith]=
            users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].SQty[_ith].length-1;
        }
		users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].SQty[_ith][users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].sellcount[_ith]] =_Avq; 
		users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].SPrice[_ith][users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].sellcount[_ith]] =_Avp;
		users[userAddresstoID[grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]]].sellcount[_ith]++; 
		
		users[userAddresstoID[grids[gridIDToNo[_GridNo]].EBaddr[_ith][ij]]].BQty[_ith] +=_Avq; 
        if (grids[gridIDToNo[_GridNo]].matchedSeller[_ith].length>grids[gridIDToNo[_GridNo]].matchedLength[_ith]){
            grids[gridIDToNo[_GridNo]].matchedBuyer[_ith][grids[gridIDToNo[_GridNo]].matchedLength[_ith]] = grids[gridIDToNo[_GridNo]].EBaddr[_ith][ij];
            grids[gridIDToNo[_GridNo]].matchedSeller[_ith][grids[gridIDToNo[_GridNo]].matchedLength[_ith]]=grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir];
            grids[gridIDToNo[_GridNo]].soldQuantity[_ith][grids[gridIDToNo[_GridNo]].matchedLength[_ith]]  =_Avq;      
            grids[gridIDToNo[_GridNo]].soldPrice[_ith][grids[gridIDToNo[_GridNo]].matchedLength[_ith]]  =_Avp;      
        }
        else{
            grids[gridIDToNo[_GridNo]].matchedBuyer[_ith].push(grids[gridIDToNo[_GridNo]].EBaddr[_ith][ij]);
            grids[gridIDToNo[_GridNo]].matchedSeller[_ith].push(grids[gridIDToNo[_GridNo]].ESaddr[_ith][ir]);
            grids[gridIDToNo[_GridNo]].soldQuantity[_ith].push(_Avq);      
            grids[gridIDToNo[_GridNo]].soldPrice[_ith].push(_Avp);            
        }
        grids[gridIDToNo[_GridNo]].matchedLength[_ith]++;
    }
